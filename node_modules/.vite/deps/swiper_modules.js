import { C as setCSSProperty, D as getWindow, E as getDocument, S as now, _ as getSlideTransformEl, a as elementChildren, b as makeElementsArray, d as elementParents, g as getRotateFix, l as elementOffset, m as elementTransitionEnd, n as classesToTokens, o as elementIndex, r as createElement, u as elementOuterSize, v as getTranslate, w as setInnerHTML, x as nextTick, y as isObject } from "./utils-CgVOMPbW.js";
...
function Zoom({ swiper, extendParams, on, emit }) {
	...
	function onTouchMove(e) {
		const isMousePan = e.pointerType === "mouse" && swiper.params.zoom.panOnMouseMove;
		...
		if (Math.max(Math.abs(image.touchesCurrent.x - image.touchesStart.x), Math.abs(image.touchesCurrent.y - image.touchesStart.y)) > 5) swiper.allowClick = false;
	...
	function zoomIn(e) {
		...
		if (!gesture.slideEl) {
			...
			if (!gesture.slideEl) if (swiper.params.virtual && swiper.params.virtual.enabled && swiper.virtual) gesture.slideEl = elementChildren(swiper.slidesEl, `.${swiper.params.slideActiveClass}`)[0];
...
function Controller({ swiper, extendParams, on }) {
	...
	on("beforeInit", () => {
		if (typeof window !== "undefined" && (typeof swiper.params.controller.control === "string" || swiper.params.controller.control instanceof HTMLElement)) {
			(typeof swiper.params.controller.control === "string" ? [...document.querySelectorAll(swiper.params.controller.control)] : [swiper.params.controller.control]).forEach((controlElement) => {
...
function History({ swiper, extendParams, on }) {
	...
	const setHistory = (key, index) => {
		...
		let value = slugify((swiper.virtual && swiper.params.virtual.enabled ? swiper.slidesEl.querySelector(`[data-swiper-slide-index="${index}"]`) : swiper.slides[index]).getAttribute("data-history"));
		...
		if (swiper.params.history.replaceState) window.history.replaceState({ value }, null, value);
		else window.history.pushState({ value }, null, value);
	};
	...
	extendParams({ hashNavigation: {
		...
		getSlideIndex(_s, hash) {
			if (swiper.virtual && swiper.params.virtual.enabled) {
				...
				return parseInt(slideWithHash.getAttribute("data-swiper-slide-index"), 10);
			}
...
function Grid({ swiper, extendParams, on }) {
	...
	const initSlides = (slides) => {
		...
		if (slidesPerView !== "auto" && fill === "row") slidesNumberEvenToRows = Math.max(slidesNumberEvenToRows, Math.floor(slidesPerView) * rows);
...
function EffectCube({ swiper, extendParams, on }) {
	...
	const recreateShadows = () => {
		...
		swiper.slides.forEach((slideEl) => {
			createSlideShadows(slideEl, Math.max(Math.min(slideEl.progress, 1), -1), isHorizontal);
		});
	};
	const setTranslate = () => {
		...
		if (params.shadow) if (isHorizontal) cubeShadowEl.style.transform = `translate3d(0px, ${swiperWidth / 2 + params.shadowOffset}px, ${-swiperWidth / 2}px) rotateX(89.99deg) rotateZ(0deg) scale(${params.shadowScale})`;
		else {
...
//#endregion
export { A11y, Autoplay, Controller, EffectCards, EffectCoverflow, EffectCreative, EffectCube, EffectFade, EffectFlip, freeMode as FreeMode, Grid, HashNavigation, History, Keyboard, Manipulation, Mousewheel, Navigation, Pagination, Parallax, Scrollbar, Thumb as Thumbs, Virtual, Zoom };
